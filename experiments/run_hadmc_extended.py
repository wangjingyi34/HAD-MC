...
def run_hadmc_full():
    """运行完整HAD-MC流程（使用扩展算法）"""
    print("=" * 80)
    print("HAD-MC Full Pipeline (Extended for Object Detection)")
    print("=" * 80)
    ...
    yolov5_dataloader = create_dataloader(
        train_path,
        imgsz=640,
        batch_size=8,  # 减小batch size以加快速度
        stride=32,
        hyp=None,
        augment=False,
        cache=False,
    ...
    # 1. 梯度敏感剪枝（使用扩展算法）
    print(f"\n[4/6] Gradient-Sensitivity Pruning (Extended)...")
    print("-" * 80)
    ...
    try:
        pruner = GradientSensitivityPruner(
            ...
            device=device,
            task_type='detection'  # 指定为检测任务
        )
        
        pruned_model = pruner.run(prune_ratio=0.3)  # 使用30%剪枝率
    ...
    # 2. 层级自适应量化（使用扩展算法）
    print(f"\n[5/6] Layer-wise Adaptive Quantization (Extended)...")
    print("-" * 80)
    ...
    # 3. 特征对齐知识蒸馏（使用扩展算法）
    print(f"\n[6/6] Feature-Aligned Knowledge Distillation (Extended)...")
    print("-" * 80)
