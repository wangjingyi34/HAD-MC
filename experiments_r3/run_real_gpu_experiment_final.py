...
Fixed version with CUDA handling and proper model dimensions
"""
...
# Device setup - handle CUDA properly
...
try:
    ...
    if torch.cuda.is_available():
        ...
        # Initialize CUDA with a dummy operation
        _ = torch.zeros(1).cuda()
        torch.cuda.empty_cache()
        torch.cuda.synchronize()

        # Set CUDA settings for compatibility
...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, num_classes=2):
        ...
        # Input: (3, 224, 224)
        # After Conv+Pool layers: (128, 28, 28) -> Linear input: 128*28*28
...
class HADMCPruner:
    ...
    def prune_conv_layer(self, conv_layer):
        ...
        with torch.no_grad():
            ...
            # PyTorch 2.3.0 compatibility: compute norm manually
            weight_norm = torch.abs(conv_layer.weight.data).sum(dim=[1, 2, 3])
    ...
    for i in range(num_samples):
        ...
        if i % 2 == 0:
            ...
            for c in range(3):
                ...
                X[i, 1] = np.maximum(X[i, 1], 0.3)  # More orange
                X[i, 2] = np.maximum(X[i, 2], 0.2)  # Some yellow
        else:
            y[i] = 1
...
def train_model(model, train_loader, test_loader, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # Sync before backward pass to prevent CUBLAS issues
            if device.type == 'cuda':
                torch.cuda.synchronize()

            loss.backward()

            # Sync after backward pass
            if device.type == 'cuda':
                torch.cuda.synchronize()
...
"""
    ...
    print("- Training: Actual training on {device}")
