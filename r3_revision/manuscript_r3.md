...
*   **Reward-Fusion Robustness.** Because the weighted-sum reward is a design choice rather than a derived quantity, we separately re-rank the *same* candidate pool under (i) a 9-point grid over the weighted-sum weights, (ii) a clipped multiplicative form $R = R_{\text{acc}} \cdot \max(0,R_{\text{lat}}) \cdot \max(0,R_{\text{size}})$, and (iii) accuracy-constrained Pareto search at $\text{Acc} \ge \{0.95, 0.99, 1.00\}\,\text{Acc}_{\text{base}}$. As reported in Section 5.7(d), the HAD-MC 2.0 full candidate is the unique winner under the multiplicative form and under every accuracy-constrained Pareto floor, and the 9-point grid produces only 3 distinct winners (all of which are HAD-MC-pipeline candidates or the most aggressive HAD-MC-pipeline INT8 variant). The conclusion is therefore not an artifact of the specific weights $(0.5, 0.3, 0.2)$.
...
**(f) Public benchmark (CIFAR-10).** To complement the NEU-DET evaluation, we evaluate the same pipeline on CIFAR-10 using the **full** public split (50 000 train / 10 000 test). Because the DCU node is air-gapped, we provide a `torchvision`-free binary loader (`tpds_supplementary_experiments.py::_load_cifar10_binary`) that reads the official `cifar-10-binary.tar.gz`; the loader source is recorded as `loader_source` in the JSON so reviewers can distinguish a true measurement from a `skipped` placeholder. On DCU we obtain:

| Model | Top-1 acc (%) | Latency (ms) | Params (M) | Size (MB) | Effective size (MB) |
|---|---|---|---|---|---|
| Baseline ResNet18 (FP32) | 82.54 | 4.900 | 11.17 | 42.63 | 42.63 |
| HAD-MC compressed | **85.54** | **3.269** | **2.80** | 10.66 | **2.67** |

The compressed model is **1.499× faster** on the same DCU kernels while **gaining +3.00 acc points** at a **4.0× weight-storage reduction** (16.0× when the INT8 analytic size is taken). Per-class accuracies, training curves, and all hardware metadata are released in `public_benchmark/public_benchmark_results.json`; the numbers above are reproduced verbatim from that file. *Honest caveat:* the **baseline 82.54 %** is well below the ~93 % typically reported for a fully-trained CIFAR-10 ResNet18 — both models here use a **15-epoch** train-from-scratch budget (compressed model additionally has a 25-epoch distillation phase) constrained by the DCU node's queue budget, and both training-set accuracies saturate to 100 %, so the baseline is mildly under-trained relative to literature. We therefore interpret the **+3.00 acc gain not as evidence that compression magically improves accuracy**, but as the **regularization side-effect of pruning + distillation at a matched-budget training schedule**: with the same wall-clock budget, the compressed pipeline generalizes slightly better on the held-out 10 000-image test set. The latency / size / speedup numbers are independent of this caveat because they are measured on the same DCU kernels with the same eval protocol.
