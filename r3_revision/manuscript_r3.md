...
The compressed model is **1.499× faster** on the same DCU kernels while **gaining +3.00 acc points** (a regularization side-effect of the pruning + distillation pipeline at this dataset size), at a **4.0× weight-storage reduction** (16.0× when the INT8 analytic size is taken). Per-class accuracies, training curves, and all hardware metadata are released in `public_benchmark/public_benchmark_results.json`; the numbers above are reproduced verbatim from that file.
